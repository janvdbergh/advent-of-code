...
class Instruction {
	...
	private boolean evaluateCondition(Map<String, Integer> registers) {
		...
		return switch (conditionOperator) {
			...
			case "==" -> conditionRegisterValue == conditionValue;
			case "!=" -> conditionRegisterValue != conditionValue;
			default -> throw new IllegalArgumentException(conditionOperator);
		};
	}
	...
	private Map<String, Integer> evaluateExpression(Map<String, Integer> registers) {
		...
		return switch (updateOperator) {
			case "dec" -> registers.put(updateRegister, updateRegisterValue - updateValue);
			case "inc" -> registers.put(updateRegister, updateRegisterValue + updateValue);
			default -> throw new IllegalArgumentException(updateOperator);
		};
	}
