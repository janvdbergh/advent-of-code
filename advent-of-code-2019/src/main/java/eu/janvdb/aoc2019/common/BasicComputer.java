...
public class BasicComputer {
	...
	private int getAddress(int index) {
		return switch (getMode(index)) {
			case POSITION -> (int) read(pc + index + 1);
			case IMMEDIATE -> pc + index + 1;
			case RELATIVE -> (int) read(pc + index + 1) + relativeBase;
			default -> throw new IllegalArgumentException(pc + "/" + read(pc));
		};
	}
