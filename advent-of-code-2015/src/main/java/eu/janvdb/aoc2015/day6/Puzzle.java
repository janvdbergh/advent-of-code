package eu.janvdb.aoc2015.day6;
...
public class Puzzle {
	...
	private static final String[] INPUT = {"turn off 660,55 through 986,197",
	...
	private static void processStep(String step) {
		Pattern pattern = Pattern.compile("([a-z ]+) ([0-9]+),([0-9]+) through ([0-9]+),([0-9]+)");
		Matcher matcher = pattern.matcher(step);
		if (!matcher.matches()) {
			throw new IllegalArgumentException(step);
		}

		Consumer<Light> action = switch (matcher.group(1)) {
			case "turn on" -> Light::turnOn;
			case "turn off" -> Light::turnOff;
		...
		performAction(x1, y1, x2, y2, action);
	}

	private static void performAction(int x1, int y1, int x2, int y2, Consumer<Light> action) {
		for (int x = x1; x <= x2; x++) {
			for (int y = y1; y <= y2; y++) {
				action.accept(lights[x * SIZE + y]);
			}
		}
	}
