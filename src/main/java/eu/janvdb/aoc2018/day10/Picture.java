...
class Picture {
	...
	void printAt(int time) {
		List<Integer> xs = movingPoints.stream().mapToInt(point -> point.getXAt(time)).boxed().collect(Collectors.toList());
		List<Integer> ys = movingPoints.stream().mapToInt(point -> point.getYAt(time)).boxed().collect(Collectors.toList());
