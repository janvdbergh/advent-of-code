...
public class Puzzle {
	...
	private static Instruction parseInstruction(String instructionStr) {
		...
		switch (parts[0]) {
			case "snd":
				return new Snd(parts[1]);
			case "rcv":
				return new Rcv(parts[1]);
			case "set":
				return new Set(parts[1], parts[2]);
			case "add":
				return new Add(parts[1], parts[2]);
			case "mul":
				return new Mul(parts[1], parts[2]);
			case "mod":
				return new Mod(parts[1], parts[2]);
			case "jgz":
				return new Jgz(parts[1], parts[2]);
			default:
				throw new IllegalArgumentException(instructionStr);
		}
	}
