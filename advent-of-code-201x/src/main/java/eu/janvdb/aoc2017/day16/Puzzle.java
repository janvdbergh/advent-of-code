...
public class Puzzle {
	...
	private static Function<ProgramSet, ProgramSet> parseInstruction(String input) {
		...
		if (spinMatcher.matches()) {
			return programSet -> programSet.spin(Integer.valueOf(spinMatcher.group(1)));
		}

		Matcher exchangeMatcher = EXCHANGE.matcher(input);
		if (exchangeMatcher.matches()) {
			return programSet -> programSet.exchange(Integer.valueOf(exchangeMatcher.group(1)), Integer.valueOf(exchangeMatcher.group(2)));
		}
